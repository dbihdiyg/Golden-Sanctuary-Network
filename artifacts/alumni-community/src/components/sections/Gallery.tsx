    ...
    <section className="relative overflow-hidden px-6 py-24">
      ...
      <div className="mx-auto max-w-6xl space-y-12">
        <div className="mx-auto max-w-2xl space-y-4 text-center">
          <p className="text-sm font-medium tracking-[0.25em] text-primary">גלריית זיכרונות</p>
          <h2 className="inline-block gold-gradient-text text-4xl font-bold md:text-5xl">רגעים שנשארים בלב</h2>
        ...
        <div className="grid auto-rows-[220px] grid-cols-1 gap-5 md:grid-cols-3">
          {photos.map((photo, index) => (
            <button
              ...
              className={`group relative overflow-hidden rounded-[2rem] border border-white/10 bg-card text-right shadow-2xl reveal-up ${photo.className}`}
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-background/10 to-transparent opacity-80 transition group-hover:opacity-95" />
              <div className="absolute bottom-0 right-0 left-0 flex items-end justify-between gap-4 p-6">
                ...
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-primary/30 bg-primary/10 text-primary backdrop-blur-md transition group-hover:scale-110 group-hover:bg-primary group-hover:text-primary-foreground">
      ...
      {selected && (
        <div
          className="fixed inset-0 z-[100] grid place-items-center bg-black/90 p-5 backdrop-blur-lg animate-in fade-in duration-300"
        ...
        >
          ...
          <figure className="max-h-[86vh] max-w-6xl overflow-hidden rounded-[2rem] border border-primary/20 bg-card shadow-[0_0_80px_rgba(202,138,4,0.18)]" onClick={(event) => event.stopPropagation()}>

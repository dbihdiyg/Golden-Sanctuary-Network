    ...
    <section className="relative overflow-hidden px-6 py-24">
      ...
      <div className="relative mx-auto max-w-4xl space-y-12">
        <div className="space-y-4 text-center">
          <p className="text-sm font-medium tracking-[0.25em] text-primary">לוח הקהילה</p>
          <h2 className="text-4xl font-bold text-white md:text-5xl">חדשות ועדכונים</h2>
          <p className="mx-auto max-w-2xl leading-relaxed text-muted-foreground">
            זרם עדכונים חי שמחזיק את כולם מחוברים למה שקורה עכשיו.
          </p>
        </div>

        <div className="relative space-y-6 before:absolute before:right-6 before:top-4 before:bottom-4 before:w-px before:bg-gradient-to-b before:from-primary before:via-primary/30 before:to-transparent">
          {feed.map((item, index) => (
            <article
              key={item.title}
            ...
            >
              <div className="absolute right-3 top-5 h-7 w-7 rounded-full border border-primary/40 bg-background shadow-[0_0_25px_rgba(202,138,4,0.22)]">
                <div className="absolute inset-2 rounded-full bg-primary" />
              </div>
              <div className="rounded-[1.75rem] border border-white/10 bg-card/70 p-6 backdrop-blur-md transition duration-300 hover:border-primary/30 hover:bg-card">

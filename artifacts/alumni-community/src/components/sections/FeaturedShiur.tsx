...
const FALLBACK: ShiurData = {
  id: 0,
  title: 'שיעור פרשת תזריע–מצורע · י״ט ניסן תשפ״ו',
  subtitle: 'הרב יצחק דוד גרוסמן שליט"א',
  audioUrl: "/shiur-tazria-metzora-tashpav.mp3",
  thumbnailUrl: "/rabbi-shiur-thumb.jpg",
  rabbiName: 'הרב יצחק דוד גרוסמן שליט"א',
  parasha: "תזריע–מצורע · תשפ״ו",
  durationLabel: "",
};

export default function FeaturedShiur() {
  const [shiur, setShiur] = useState<ShiurData | null>(null);
  ...
  useEffect(() => {
    fetch("/api/cms/featured-shiur")
      .then(r => r.ok ? r.json() : null)
      .then(data => setShiur(data ?? FALLBACK))
      .catch(() => setShiur(FALLBACK));
  }, []);
  ...
  if (dismissed || !shiur || !shiur.audioUrl) return null;

  const thumb = shiur.thumbnailUrl || "/rabbi-shiur-thumb.jpg";
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        <img src={thumb} alt={shiur.rabbiName}
        ...
        <div className={`absolute inset-0 transition-opacity duration-700 ${playing ? "opacity-100" : "opacity-0"}`}
          style={{ background: "radial-gradient(ellipse 60% 80% at 60% 50%, rgba(245,192,55,0.18) 0%, transparent 70%)",
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-10 md:px-6 md:py-14">
        <div className="flex flex-col md:flex-row items-center md:items-stretch gap-8 md:gap-14">
          <div className="shrink-0 relative hidden md:block">
            <div className="h-52 w-52 rounded-[2rem] overflow-hidden border-2 shadow-[0_0_60px_rgba(245,192,55,0.25),0_30px_80px_rgba(0,0,0,0.6)]"
              style={{ borderColor: "rgba(245,192,55,0.5)" }}>
              <img src={thumb} alt={shiur.rabbiName} loading="lazy" decoding="async" className="h-full w-full object-cover object-top" />
            </div>
            {playing && <div className="absolute inset-0 rounded-[2rem] border-2 border-primary/60 animate-ping opacity-30 pointer-events-none" />}
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center gap-5 text-right">
            <div className="flex items-center gap-3 flex-wrap">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/40 bg-primary/12 px-3 py-1 text-xs font-black text-primary animate-pulse">
                <Radio className="h-3 w-3" />שיעור חדש
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/8 px-3 py-1 text-xs text-muted-foreground">
                <Headphones className="h-3 w-3" />{listeners} מאזינים כעת
              </span>
              {shiur.parasha && (
            ...
            <div>
              <h2 className="font-serif text-2xl md:text-3xl font-black text-white leading-snug">{shiur.rabbiName}</h2>
              <p className="mt-1 text-muted-foreground text-sm md:text-base">{shiur.title}</p>
            </div>
              ...
              {Array.from({ length: BARS }).map((_, i) => {
                ...
                return (
                  <div key={i} className="rounded-full transition-all" style={{
                    width: 5, height: `${base * 100}%`,
                    background: isActive ? "hsl(var(--primary))"
                      : playing ? `rgba(245,192,55,${0.3 + 0.5 * base})`
            ...
            <div className="space-y-1.5">
              <div className="h-1.5 w-full cursor-pointer rounded-full overflow-hidden"
                style={{ background: "rgba(255,255,255,0.12)" }} onClick={seek}>
                <div className="h-full rounded-full transition-all duration-100"
                  style={{ width: `${progress * 100}%`, background: "linear-gradient(to left, hsl(var(--primary)), rgba(245,192,55,0.6))" }} />
              </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button onClick={toggle}
                ...
                style={{ background: "linear-gradient(135deg, hsl(var(--primary)) 0%, rgba(245,164,0,0.85) 100%)", color: "#0F1A2B" }}>
                {playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                {playing ? "השהה" : "הקשב עכשיו"}
              </button>
              <a href={shiur.audioUrl} download
                ...
                <Download className="h-4 w-4" />הורד שיעור
              </a>
            </div>
          </div>
        </div>
      ...
      <div className="absolute inset-x-0 bottom-0 h-px"
        style={{ background: "linear-gradient(to right, transparent, rgba(245,192,55,0.4), transparent)" }} />

      <style>{`
        @keyframes bar-bounce { from { transform: scaleY(0.55); } to { transform: scaleY(1.15); } }
      `}</style>
    </section>
  );
}

...
export default function CategoryHub() {
  return (
    <section className="relative overflow-hidden px-4 py-12 md:py-24" id="categories">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_16%,rgba(0,19,164,0.3),transparent_32%),radial-gradient(circle_at_12%_72%,rgba(245,192,55,0.16),transparent_30%)]" />
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-l from-transparent via-primary/55 to-transparent" />

      <div className="relative mx-auto max-w-7xl space-y-8 md:space-y-14">
        <div className="mx-auto max-w-4xl text-center">
          <p className="text-xs font-black tracking-[0.34em] text-blue-brand md:text-sm">מיד אחרי הכניסה</p>
          <h2 className="mt-3 gold-gradient-text text-3xl font-black leading-tight md:mt-4 md:text-7xl">כל שערי הקהילה</h2>
        </div>
          ...
          {categories.map((category) => {
            ...
            return (
              <Link
                key={category.title}
                href={category.href}
                className="group flex flex-col items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.05] p-4 text-center transition active:scale-95"
              >
                <span className="grid h-12 w-12 place-items-center rounded-xl border border-primary/35 bg-primary/12 text-primary transition group-active:bg-primary group-active:text-primary-foreground">
        ...
        <div className="hidden md:grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {categories.map((category, index) => {
            ...
            return (
              ...
              >
                <img src={category.image} alt={category.title} className="absolute inset-0 h-full w-full object-cover opacity-40 transition duration-1000 group-hover:scale-110 group-hover:opacity-60" />

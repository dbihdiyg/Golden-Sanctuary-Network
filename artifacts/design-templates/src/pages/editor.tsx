...
export default function Editor() {
  ...
  useEffect(() => {
    ...
    // ── Comprehensive slot → SlotStyle extraction ─────────────────────────────
    // AdminSlot extends SlotStyle, so every style field the admin set is already
    // present on the slot JSON that the DB returns. We copy ALL known keys so
    // nothing the admin designed is silently dropped.
    const ALL_STYLE_KEYS: (keyof SlotStyle)[] = [
      "fontFamily", "fontSize", "color", "bold", "italic", "underline",
      "letterSpacing", "lineHeight", "opacity", "rotation", "skewX", "skewY",
      "shadow", "shadowX", "shadowY", "shadowBlur", "shadowColor",
      "glow", "glowColor", "glowRadius", "glowIntensity",
      "extrudeEnabled", "extrudeDepth", "extrudeColor", "extrudeAngle",
      "longShadowEnabled", "longShadowLength", "longShadowColor", "longShadowAngle",
      ...
      "zIndex",
      "preset3D", "depth3D", "lightAngle3D", "shadowStr3D", "highlight3D", "glow3D",
      "mode3D", "material3D", "depth3DEngine", "bevel3D",
      "cameraAngleX", "cameraAngleY", "autoRotate3D",
    ];
    ...
    (template.slots || []).filter(s => !SYSTEM_SLOT_IDS.has(s.id)).forEach(s => {
      ...
      // Copy every known SlotStyle key that is present on the slot
      ALL_STYLE_KEYS.forEach(key => {
        if (a[key] !== undefined && a[key] !== null) (style as any)[key] = a[key];
      });

      // fontSizePx (admin field) → fontSize (SlotStyle field)
      if (style.fontSize == null && a.fontSizePx != null) style.fontSize = a.fontSizePx;

      // Normalise warpAmount from legacy arcDegrees
      if (style.warpType && style.warpType !== "none" && style.warpAmount == null && a.arcDegrees != null) {
        style.warpAmount = Math.abs(a.arcDegrees);
      }

      if (Object.keys(style).length > 0) initSS[s.id] = style;
